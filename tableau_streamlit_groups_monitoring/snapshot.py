...
def fetch_all_group_members(server: TSC.Server) -> tuple[list[dict], list[dict], dict[str, str]]:
    """Fetch every group, its full member list, and the group inventory.

    Returns (members, groups, group_id_to_name). The name map is reused by
    the workbook-permissions fold, since PermissionsRule grantees carry an
    ID but not a name. The groups list captures every group on the server —
    including zero-member ones, which `members` cannot represent — so the
    Access Audit page can surface orphan groups.
    """
    members = []
    groups: list[dict] = []
    ...
    for group in all_groups:
        ...
        groups.append({
            ...
            "domain_name": getattr(group, "domain_name", "") or "",
        })
    ...
    return members, groups, group_id_to_name
...
def take_snapshot():
    ...
    try:
        ...
        with server.auth.sign_in(auth):
            members, groups, group_id_to_name = fetch_all_group_members(server)
        ...
        print(f"Captured {len(groups)} groups, {len(members)} total memberships")
        ...
        db.insert_groups(conn, snapshot_id, groups)
